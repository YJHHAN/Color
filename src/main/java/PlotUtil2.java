...
public class PlotUtil2 {
    ...
    //추가시작
/*
    public static void plotTrainingData2(INDArray features, INDArray labels, INDArray backgroundIn, INDArray backgroundOut, int nDivisions) throws Exception {
        double[] mins = backgroundIn.min(0).data().asDouble();
        double[] maxs = backgroundIn.max(0).data().asDouble();
    ...
    private static JFreeChart createChart_3(XYZDataset dataset, double[] mins, double[] maxs, int nPoints, XYDataset xyData) {
        NumberAxis xAxis = new NumberAxis("X");
        xAxis.setRange(mins[0],maxs[0]);
        ...
        NumberAxis zAxis = new NumberAxis("Z");
        zAxis.setRange(mins[2], maxs[2]);
